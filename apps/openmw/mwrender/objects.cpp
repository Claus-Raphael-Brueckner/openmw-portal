...
namespace MWRender
{
    ...
    void Objects::insertModel(const MWWorld::Ptr& ptr, const std::string& mesh, bool allowLight)
    {
        ...
        // Portal doors: suppress the mesh and create a portal surface instead.
        if (mPortalManager && ptr.getClass().isDoor() && mPortalManager->tryCreatePortal(ptr))
        {
            // Register an empty animation so the door lifecycle is managed via mObjects.
            mObjects.emplace(ptr.mRef, osg::ref_ptr<ObjectAnimation>(
                new ObjectAnimation(ptr, std::string{}, mResourceSystem, false, false)));
            return;
        }
